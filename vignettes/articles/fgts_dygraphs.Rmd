...
functionality, both for flexibility and speed.\
...
-   Note that column specifications are flexible, i.e.
    `hidecols="TLT;EEM"` or `hidecols=c("TLT","EEM")` can also be used.
-   Also, notice that focusing in the displayed date range reduces the
    ...
    in the lower left corner box, it will be smoothed and the last value
...
-   The data series have the colors same colors they would have if they
    ...
    series out before sending to `fgts_dygraph()` by (e.g.) passing in
    `eqtypx |> dplyr::select(date,IBM,QQQ)` as `indta`.
...
used in addition to adding confidence bounds include \* Showing
positioning, momentum, or sentiment as (proportional) shaded areas
underneath or over a series. \* SHowing VAR bounds for unit investments.
\* Showing statistical significance to exogenous time series. \* Showing
...
the package has the rolling 66 dayp-value of regressing returns of `TLT`
agasint `QQQ`:
...
-   **Internal dataset**: The package maintains contains a prepackaged
    ...
    with the package are `fedmoves` and `regm`. An example of both is
    shown below:

```{r}
smalldta <- eqtypx |> filter(date>=as.Date("2023-01-01")) |> select(date,TLT,EEM)
...
fg_get_dates_of_interest("fedmoves") |> slice_tail(n=2)
```

-   **Seasonal factors** : Regularly recurring dates can also be added.
    They can either be absolute, (e.g. equity option expirations) or
    relative (e.g. dates with same day of the quarter as the end date of
    the series).
...
-   **Statistically determined dates** can be added. There are few
    ...
    turning points are currently implemented, as in

```{r}
fgts_dygraph(smalldta,title="With Turning Points",rebase=",100",roller=1,events="tp,5")
```
...
toplot <- rbind(eqtypx_melt |> filter(variable=="QQQ"), consumer_sent |> transmute(date,variable=symbol,value=price))
...
Note that mutiple event sets can be `rbind`-ed togehter, and the color
...
```{r}
head(ratings_db,3)
copdta <- nomfxdta |> filter(variable=="COP")
...
Time series plotted in `fgts_dygraph`s can also be extended beyond the
...
Getting those foreasts into the dygraph requires an included `broom`
like function called `fg_predict()` which takes a somewhat complicated
data structure and creates new series whch are then added to the
original data.

Alternatively, if you would like a `data.frame` with the actual
forecasts, use `sw_sweep()` from
[Sweep](https://business-science.github.io/sweep/articles/SW00_Introduction_to_sweep.html),
and then forward on to another included helper `fg_sweep()`. An example
to predict QQQ with `ets()` is shown below. First, we show the format we
...
fgts_dygraph(toplot,title="Rebased With Forecasts",roller=1,dtstartfrac=0.6,rebase="2024-01-01,100",forecast_ds=fcst_in)
```
...
* Adding other central bank moves, e.g. Brazil's COPOM decisions.  Downloading historical data 
from [BCB](https://www.bcb.gov.br/en/monetarypolicy/historicaldata), we can do

```{r}
selic<-data.table::fread("./inst/extdata/selic_historical_rates.csv",skip=1,select=c(2,5),col.names=c("date","tgt"))
...
```{r}
fgts_dygraph(filter(nomfxdta,variable=="BRL"),title="BRL w/ COPOM",events="doi,COPOM")
```
