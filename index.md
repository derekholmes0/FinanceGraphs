...
changed (again persistently) using the
...
![Hi/Lo series, used to show currency values against local
peers](reference/figures/README-simple_example3-1.png)
...
Those are customizable using the
...
![Graph of Equity Prices showing Event
styles](reference/figures/README-Events1-1.png) Several types of events
are predefined in
...
`event_ds` parameter. The details are in
[`fgts_dygraph()`](https://derekholmes0.github.io/FinanceGraphs/reference/fgts_dygraph.md),
but the basic columns are a start date date, a text label, and if
applicable an end date. Only events within the dates of the original
...
![Graph of equity prices showing sentiment
ranges](reference/figures/README-Events2-1.png) Current event helpers
are:

| Function | Description |
|:---|:---|
| [`fg_findTurningPoints()`](https://derekholmes0.github.io/FinanceGraphs/reference/fg_findTurningPoints.md) | Statistically identify turning points in a series |
...
| [`fg_tq_divs()`](https://derekholmes0.github.io/FinanceGraphs/reference/Event_Helpers.md) | Add dividend events from TidyQuant dividend data |
| [`fg_av_earnings()`](https://derekholmes0.github.io/FinanceGraphs/reference/Event_Helpers.md) | Add earnings events from AlphaVantage earnings data |
...
![Adding forecasts to original
data](reference/figures/README-Forecasts1-1.png)
...
| Function | Description |
|:---|:---|
| [`fg_sweep()`](https://derekholmes0.github.io/FinanceGraphs/reference/fg_sweep.md) | Converts [tidy forecast objects](https://business-science.github.io/sweep/) to `forecast_ds` form. |
| [`fg_prophet()`](https://derekholmes0.github.io/FinanceGraphs/reference/fg_prophet.md) | Converts [Prophet](https://business-science.github.io/sweep/) to `forecast_ds` form. |
...
Any changes made to aesthetics colors will persist across loads of the
package (unless `persist=FALSE` is specified). To see what aesthetics
are used for any particular function, use
...
development of this package), use
[`fg_update_dates_of_interest()`](https://derekholmes0.github.io/FinanceGraphs/reference/fg_dates_of_interest.md).
...
turn these on or off. The function
[`fg_sync_group()`](https://derekholmes0.github.io/FinanceGraphs/reference/fg_sync_group.md)
either returns the current group name if called with no parameters, sets
the group name with a string, or turns off synchronization with a call
`fg_sync_group(NULL)`.
...
tries to be a concise wrapper around the very comprehensive
[ggplot2](https://ggplot2.tidyverse.org/) graphics framework.
`ggplot2()` is a great ecosystem, but requires quite a bit of verbiage
to get from idea to presentable graph quickly. The approach used here is
...
[`fgts_dygraph()`](https://derekholmes0.github.io/FinanceGraphs/reference/fgts_dygraph.md),
date partitions can be custom regimes managed by
[`fg_update_dates_of_interest()`](https://derekholmes0.github.io/FinanceGraphs/reference/fg_dates_of_interest.md).
...
![Scatterplot with two categories and recent
highlighting.](reference/figures/README-Scatter1-1.png) One important
feature of the static plots in this package to note is that they include
options for switching the colors of large numbers of groups or points
from discrete values to continuous values from the
[RColorBrewer](https://colorbrewer2.org) package.

[`fg_scatplot()`](https://derekholmes0.github.io/FinanceGraphs/reference/fg_scatplot.md)
does not require dates in the input data, and can be useful for
communicating comparative analyses as well. The `mtcars` dataset shows
how we can add a lot of information to a basic text scatterplot.
...
![Scatterplot with text and linear
models](reference/figures/README-Scatter2-1.png)
...
or a violin plot. The function
[`fg_tsboxplot()`](https://derekholmes0.github.io/FinanceGraphs/reference/fg_tsboxplot.md)
provides a flexible way to split several time series into time
categories. Usually, those categories are time periods with increasing
horizons, but this function allows for arbitrary periods managed by
[`fg_update_dates_of_interest()`](https://derekholmes0.github.io/FinanceGraphs/reference/fg_dates_of_interest.md).
...
prices](reference/figures/README-Boxplot1-1.png)

Like
[`fgts_dygraph()`](https://derekholmes0.github.io/FinanceGraphs/reference/fgts_dygraph.md),
the function can take data in both wide and long formats. Longer formats
have the advantage of adding more categorical levels to our graphs. For
example, we can plotting recent ranges of the monthly FX Real Exchange
Rate dataset in the following way. Here, we split the data into buckets
of last 20pct of the months, the first half the months and the last. We
will also show the last observation, hide the boxplot whiskers, and
reorder the currencies by their relative weakness.
...
Rates](reference/figures/README-Boxplot2-1.png)
...
time relative to the event. The function
[`fg_eventStudy()`](https://derekholmes0.github.io/FinanceGraphs/reference/fg_eventStudy.md)
integrates a set of dates with a set of time series to plot their
relative behavior over the business days before and after each event. It
is designed to show reasonably large sets of assets or event dates by
switching discrete color scales to gradient scales after a user
specified number.
...
![Relative rate moves around recent Fed
Cuts](reference/figures/README-EventStudies1-1.png)
...
![Scatter plot of moves before and after Fed Rate
Cuts](reference/figures/README-EventStudies2-1.png)
